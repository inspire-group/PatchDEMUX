...
def main():
    ...
    foldername = f"dump/{val_status}/{todaystring}/trial_{args.trial}_{args.trial_type}_thre_{(int)(args.thre * 100)}percent/gpu_world_id_{args.world_gpu_id}/"
...
def validate_multi(model, val_loader, classes_list, args, mask_list_fr = None):
    ...
    for batch_index, (input, target) in enumerate(val_loader):
        ...
        print(f"target shape is {target.shape}", flush=True)
        print(f"pred shape is {pred.shape}", flush=True)

        print(f"target type is {type(target)}", flush=True)
        print(f"pred type is {type(pred)}", flush=True)

        print(f"target sum is {target.sum()}", flush=True)
        print(f"pred sum is {pred.sum()}", flush=True)
