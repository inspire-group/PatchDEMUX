...
parser.add_argument('-b', '--batch-size', default=32, type=int, help='mini-batch size (default: 16)')
...
def validate_multi(model, val_loader, classes_list, mask_list, args):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        all_preds = np.zeros([32, num_masks * num_masks, num_classes])

        for i, mask1 in enumerate(mask_list):
            mask1 = mask1.reshape(1, 1, *mask1.shape)
            for j, mask2 in enumerate(mask_list):
                mask2 = mask2.reshape(1, 1, *mask2.shape)
    ...
    np.savez("certified_metrics", metrics.TP, metrics.TN, metrics.FN, metrics.FP)

    # Save the precision and recall metrics
    with open("performance_metrics.txt", "w") as f:
        f.write(f"Overall precision: {precision_o:.2f} \t Overall recall: {recall_o:.2f}\n")
        f.write(f"Average class precision: {precision_c:.2f} \t Average class recall: {recall_c:.2f}")
